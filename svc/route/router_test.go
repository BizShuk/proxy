...
func TestRouterResolve(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.model, func(t *testing.T) {
			got, err := router.Resolve(model.FORMAT_OPENAI_CHAT, tc.model)
			if tc.wantErr {
				var proxyErr *model.ProxyError
				require.ErrorAs(t, err, &proxyErr)
				assert.Equal(t, model.ERROR_UNKNOWN_MODEL, proxyErr.Kind)
				return
			}
			require.NoError(t, err)
...
func TestRouterResolveRejectsAmbiguousUnqualifiedModel(t *testing.T) {
	...
	_, err = router.Resolve(model.FORMAT_OPENAI_RESPONSES, "shared-model")
	var proxyErr *model.ProxyError
	require.ErrorAs(t, err, &proxyErr)
	assert.Equal(t, model.ERROR_UNKNOWN_MODEL, proxyErr.Kind)
}

func formatPtr(value model.Format) *model.Format {
	return &value
}
