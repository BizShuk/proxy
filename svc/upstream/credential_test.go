...
func TestCredentialResolverRefreshesWithRequestContextAndSaves(t *testing.T) {
	...
	store := newFakeCredentialStore(t, []*authmodel.Credential{{
		Provider: "openai", Kind: authmodel.KIND_OAUTH, AccessToken: "old", RefreshToken: "refresh",
		ExpiresAt: time.Now().Add(-time.Minute),
	}})
	authenticator := &fakeAuthenticator{refresh: &authmodel.Credential{
		Provider: "openai", Kind: authmodel.KIND_OAUTH, AccessToken: "new", RefreshToken: "rotated",
...
func TestCredentialResolverSelectsActiveThenAlphabeticFallback(t *testing.T) {
	creds := []*authmodel.Credential{
		{Provider: "openai", Kind: authmodel.KIND_API_KEY, Account: "zeta", APIKey: "zeta-key"},
		{Provider: "anthropic", Kind: authmodel.KIND_API_KEY, Account: "other", APIKey: "other-key"},
		{Provider: "openai", Kind: authmodel.KIND_API_KEY, Account: "alpha", APIKey: "alpha-key"},
	}
...
func TestCredentialResolverUsesProviderEnvironmentFallback(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.provider, func(t *testing.T) {
			...
			assert.Equal(t, authmodel.KIND_API_KEY, cred.Kind)
...
func TestCredentialResolverReturnsUnavailableOnRefreshOrSaveFailure(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			name: "save failure",
			authenticator: &fakeAuthenticator{refresh: &authmodel.Credential{
				Provider: "openai", Kind: authmodel.KIND_OAUTH, AccessToken: "rotated-access", RefreshToken: "rotated-refresh",
				ExpiresAt: time.Now().Add(time.Hour),
			}},
			saveErr: saveErr,
		},
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			store := newFakeCredentialStore(t, []*authmodel.Credential{{
				Provider: "openai", Kind: authmodel.KIND_OAUTH, AccessToken: "expired", RefreshToken: "refresh",
				ExpiresAt: time.Now().Add(-time.Minute),
			}})
...
type fakeCredentialStore struct {
	dir     string
	creds   []*authmodel.Credential
	saved   []*authmodel.Credential
	listErr error
	loadErr error
	saveErr error
}

func newFakeCredentialStore(t *testing.T, creds []*authmodel.Credential) *fakeCredentialStore {
	t.Helper()
...
type fakeAuthenticator struct {
	refresh        *authmodel.Credential
	refreshErr     error
	refreshContext context.Context
}

func (a *fakeAuthenticator) Provider() string     { return "openai" }
func (a *fakeAuthenticator) Kind() authmodel.Kind { return authmodel.KIND_OAUTH }

func (a *fakeAuthenticator) Login(context.Context) (*authmodel.Credential, error) {
	return nil, errors.New("unexpected login")
}

func (a *fakeAuthenticator) Refresh(ctx context.Context, _ *authmodel.Credential) (*authmodel.Credential, error) {
	a.refreshContext = ctx
	return a.refresh, a.refreshErr
}

func (a *fakeAuthenticator) Verify(context.Context, *authmodel.Credential) (*authmodel.VerifyResult, error) {
