...
func TestClientDoNeverFollowsRedirects(t *testing.T) {
	...
	credentials := []struct {
	...
	}{
		{
			...
			credential: &authmodel.Credential{Provider: "xai", Kind: authmodel.KIND_API_KEY, APIKey: "secret"},
		},
		{
			name:       "x-api-key",
			profileID:  "anthropic",
			target:     model.FORMAT_ANTHROPIC_MESSAGES,
			credential: &authmodel.Credential{Provider: "anthropic", Kind: authmodel.KIND_API_KEY, APIKey: "secret"},
		},
	}
	for _, redirect := range redirects {
		for _, credential := range credentials {
			t.Run(redirect.name+"/"+credential.name, func(t *testing.T) {
				...
				response, err := client.Do(context.Background(), profile, credential.credential, model.RequestEnvelope{
...
func TestClientDoUsesProfileEndpointAndSanitizedHeaders(t *testing.T) {
	...
	require.NoError(t, err)
	resp, err := client.Do(context.Background(), profile, &authmodel.Credential{Provider: "xai", Kind: authmodel.KIND_API_KEY, APIKey: "secret"}, model.RequestEnvelope{
...
func TestClientDoJoinsBasePathAndAllowsCredentialOverride(t *testing.T) {
	...
	resp, err := client.Do(context.Background(), profile, &authmodel.Credential{
		Provider: "minimax", Kind: authmodel.KIND_API_KEY, APIKey: "secret",
	}, model.RequestEnvelope{TargetFormat: model.FORMAT_ANTHROPIC_MESSAGES, Body: []byte(`{}`)})
	require.NoError(t, err)
	require.NoError(t, resp.Body.Close())
	...
	resp, err = client.Do(context.Background(), profile, &authmodel.Credential{
		Provider: "minimax", Kind: authmodel.KIND_API_KEY, APIKey: "secret", BaseURL: server.URL + "/gateway",
	}, model.RequestEnvelope{TargetFormat: model.FORMAT_ANTHROPIC_MESSAGES, Body: []byte(`{}`)})
	require.NoError(t, err)
	require.NoError(t, resp.Body.Close())
...
func TestClientDoRejectsUnsafeBaseURLs(t *testing.T) {
	...
	for _, rawURL := range tests {
		t.Run(rawURL, func(t *testing.T) {
			...
			_, err := client.Do(context.Background(), profile, &authmodel.Credential{
				Provider: "xai", Kind: authmodel.KIND_API_KEY, APIKey: "secret",
			}, model.RequestEnvelope{TargetFormat: model.FORMAT_OPENAI_RESPONSES, Body: []byte(`{}`)})
...
func TestClientDoAppliesProviderSpecificHeaders(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Anthropic API key", profileID: "anthropic", target: model.FORMAT_ANTHROPIC_MESSAGES,
			credential: &authmodel.Credential{Provider: "anthropic", Kind: authmodel.KIND_API_KEY, APIKey: "anthropic-key"},
		...
		{
			name: "MiniMax", profileID: "minimax", target: model.FORMAT_ANTHROPIC_MESSAGES,
			credential: &authmodel.Credential{Provider: "minimax", Kind: authmodel.KIND_API_KEY, APIKey: "minimax-key"},
			assert: func(t *testing.T, header http.Header) {
		...
		{
			name: "OpenAI API", profileID: "openai-api", target: model.FORMAT_OPENAI_RESPONSES,
			credential: &authmodel.Credential{Provider: "openai", Kind: authmodel.KIND_API_KEY, APIKey: "openai-key"},
			assert: func(t *testing.T, header http.Header) {
				assert.Equal(t, "Bearer openai-key", header.Get("Authorization"))
			},
		},
		{
			name: "Codex OAuth", profileID: "openai-codex-oauth", target: model.FORMAT_OPENAI_RESPONSES, stream: true,
			credential: &authmodel.Credential{Provider: "openai", Kind: authmodel.KIND_OAUTH, AccessToken: "codex-token", AccountID: "acct-uuid"},
			assert: func(t *testing.T, header http.Header) {
		...
		{
			name: "xAI", profileID: "xai", target: model.FORMAT_OPENAI_RESPONSES,
			credential: &authmodel.Credential{Provider: "xai", Kind: authmodel.KIND_API_KEY, APIKey: "xai-key"},
			assert: func(t *testing.T, header http.Header) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			resp, err := client.Do(context.Background(), profile, tc.credential, model.RequestEnvelope{
...
func TestClientDoOmitsEmptyCodexAccountID(t *testing.T) {
	...
	resp, err := client.Do(context.Background(), profile, &authmodel.Credential{
		Provider: "openai", Kind: authmodel.KIND_OAUTH, AccessToken: "token",
	}, model.RequestEnvelope{TargetFormat: model.FORMAT_OPENAI_RESPONSES, Stream: true, Body: []byte(`{}`)})
	require.NoError(t, err)
	require.NoError(t, resp.Body.Close())
}
...
func TestClientDoUsesRequestTimeoutAndCancelsOnBodyClose(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			resp, err := client.Do(context.Background(), profile, &authmodel.Credential{
				Provider: "xai", Kind: authmodel.KIND_API_KEY, APIKey: "secret",
			}, model.RequestEnvelope{TargetFormat: model.FORMAT_OPENAI_RESPONSES, Stream: tc.stream, Body: []byte(`{}`)})
			require.NoError(t, err)
			require.NoError(t, resp.Body.Close())
			select {
			case <-captured.Done():
...
func TestClientDoPropagatesCallerCancellation(t *testing.T) {
	...
	go func() {
		_, err := client.Do(ctx, profile, &authmodel.Credential{
			Provider: "xai", Kind: authmodel.KIND_API_KEY, APIKey: "secret",
		}, model.RequestEnvelope{TargetFormat: model.FORMAT_OPENAI_RESPONSES, Body: []byte(`{}`)})
		result <- err
	}()
...
func TestClientDoMapsTransportFailures(t *testing.T) {
	tests := []struct {
		...
		{name: "deadline", err: context.DeadlineExceeded, wantKind: model.ERROR_TIMEOUT, status: http.StatusGatewayTimeout},
		{name: "network timeout", err: timeoutError{}, wantKind: model.ERROR_TIMEOUT, status: http.StatusGatewayTimeout},
		{name: "generic upstream", err: errors.New("dial refused"), wantKind: model.ERROR_UPSTREAM, status: http.StatusBadGateway},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			_, err = client.Do(context.Background(), defaultProfile(t, "xai"), &authmodel.Credential{
				Provider: "xai", Kind: authmodel.KIND_API_KEY, APIKey: "secret",
			}, model.RequestEnvelope{TargetFormat: model.FORMAT_OPENAI_RESPONSES, Body: []byte(`{}`)})
			var proxyErr *model.ProxyError
			require.ErrorAs(t, err, &proxyErr)
...
func TestClientCountTokensUsesNativeEndpointAndTimeout(t *testing.T) {
	...
	resp, err := client.CountTokens(context.Background(), profile, &authmodel.Credential{
		Provider: "anthropic", Kind: authmodel.KIND_API_KEY, APIKey: "anthropic-key",
	}, model.RequestEnvelope{TargetFormat: model.FORMAT_ANTHROPIC_MESSAGES, Body: []byte(`{"model":"claude-3-5-sonnet-latest","messages":[]}`)})
	require.NoError(t, err)
...
func TestClientCountTokensRejectsUnsupportedProfile(t *testing.T) {
	...
	_, err = client.CountTokens(context.Background(), defaultProfile(t, "xai"), &authmodel.Credential{
		Provider: "xai", Kind: authmodel.KIND_API_KEY, APIKey: "secret",
	}, model.RequestEnvelope{TargetFormat: model.FORMAT_OPENAI_RESPONSES, Body: []byte(`{}`)})
	var proxyErr *model.ProxyError
	require.ErrorAs(t, err, &proxyErr)
