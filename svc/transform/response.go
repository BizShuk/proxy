...
func DecodeUpstreamError(status int, headers http.Header, body []byte) *model.ProxyError {
...
func upstreamErrorKind(status int) model.ErrorKind {
	switch status {
	case http.StatusUnauthorized, http.StatusForbidden:
		return model.ERROR_AUTH
	case http.StatusTooManyRequests:
		return model.ERROR_RATE_LIMIT
	case http.StatusRequestTimeout, http.StatusGatewayTimeout:
		return model.ERROR_TIMEOUT
	case http.StatusBadGateway, http.StatusServiceUnavailable:
		return model.ERROR_UNAVAILABLE
	default:
		return model.ERROR_UPSTREAM
	}
}
...
func responseModel(envelope model.ResponseEnvelope, fallback string) string {
...
func generatedID(exchange model.Exchange, fallback string) string {
...
func cachedTokenLoss(creationTokens int) []model.SemanticLoss {
	if creationTokens == 0 {
		return nil
	}
