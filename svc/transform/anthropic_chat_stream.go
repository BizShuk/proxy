...
func NewChatToAnthropicStream(exchange model.Exchange) (StreamTransform, error) {
...
func (s *chatToAnthropicStream) Push(ctx context.Context, frame model.SSEFrame) ([]model.SSEFrame, error) {
...
func (s *chatToAnthropicStream) Close(ctx context.Context) ([]model.SSEFrame, error) {
	if err := ctx.Err(); err != nil {
		return nil, err
	}
	if !s.terminal {
...
func (s *chatToAnthropicStream) pushError(raw json.RawMessage) ([]model.SSEFrame, error) {
	if !json.Valid(raw) {
...
func (s *chatToAnthropicStream) pushChoice(choice chat.StreamChoice) ([]model.SSEFrame, error) {
	var output []model.SSEFrame
...
func (s *chatToAnthropicStream) ensureMessageStart() []model.SSEFrame {
	if s.started {
		return nil
	}
	s.started = true
	...
	return []model.SSEFrame{anthropicChatStreamFrame("message_start", payload)}
}

func (s *chatToAnthropicStream) pushThinking(delta string) ([]model.SSEFrame, error) {
...
func (s *chatToAnthropicStream) pushText(delta string) ([]model.SSEFrame, error) {
	if len(s.tools) > 0 {
...
func (s *chatToAnthropicStream) pushTool(call chat.StreamToolCall) ([]model.SSEFrame, error) {
...
func (s *chatToAnthropicStream) finish() ([]model.SSEFrame, error) {
	output := append(s.closeThinking(), s.closeText()...)
...
func (s *chatToAnthropicStream) closeThinking() []model.SSEFrame {
	if !s.thinkingOpen {
		return nil
	}
	s.thinkingOpen = false
	return []model.SSEFrame{anthropicChatStreamFrame("content_block_stop", map[string]any{
		"type": "content_block_stop", "index": s.thinkingBlock,
	})}
}

func (s *chatToAnthropicStream) closeText() []model.SSEFrame {
	if !s.textOpen {
		return nil
	}
	s.textOpen = false
...
func anthropicChatStreamFrame(event string, value any) model.SSEFrame {
...
func NewAnthropicToChatStream(exchange model.Exchange) (StreamTransform, error) {
...
func (s *anthropicToChatStream) Push(ctx context.Context, frame model.SSEFrame) ([]model.SSEFrame, error) {
	if err := ctx.Err(); err != nil {
		return nil, err
	}
	if s.terminal {
...
func (s *anthropicToChatStream) Close(ctx context.Context) ([]model.SSEFrame, error) {
	if err := ctx.Err(); err != nil {
		return nil, err
	}
	if !s.terminal {
...
func (s *anthropicToChatStream) start(event anthropicToChatEvent) ([]model.SSEFrame, error) {
	if s.started {
	...
	return []model.SSEFrame{s.chatFrame(chat.StreamDelta{Role: "assistant"}, "", nil)}, nil
}

func (s *anthropicToChatStream) startBlock(event anthropicToChatEvent) ([]model.SSEFrame, error) {
	if !s.started {
	...
	case "tool_use":
		...
		return []model.SSEFrame{s.chatFrame(delta, "", nil)}, nil
	default:
		return nil, protocolFailure(fmt.Errorf("unsupported Anthropic content block %q", event.ContentBlock.Type))
	}
}

func (s *anthropicToChatStream) pushBlockDelta(event anthropicToChatEvent) ([]model.SSEFrame, error) {
...
func (s *anthropicToChatStream) pushMessageDelta(event anthropicToChatEvent) ([]model.SSEFrame, error) {
	if !s.started {
	...
	return []model.SSEFrame{s.chatFrame(chat.StreamDelta{}, anthropicToChatStop(event.Delta.StopReason), usage)}, nil
}

func (s *anthropicToChatStream) stopMessage() ([]model.SSEFrame, error) {
	if !s.stopSeen {
		return nil, protocolFailure(fmt.Errorf("Anthropic message_stop arrived before stop_reason"))
	}
	s.terminal = true
	return []model.SSEFrame{{Data: []byte("[DONE]")}}, nil
}

func (s *anthropicToChatStream) pushError(raw json.RawMessage) ([]model.SSEFrame, error) {
	...
	s.terminal = true
	return []model.SSEFrame{{Data: data}, {Data: []byte("[DONE]")}}, nil
}

func (s *anthropicToChatStream) chatFrame(delta chat.StreamDelta, finishReason string, usage *chat.Usage) model.SSEFrame {
