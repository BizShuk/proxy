...
type StreamCollector interface {
	Push(context.Context, model.SSEFrame) error
	Close(context.Context) (model.TransformResult, error)
}

// NewStreamCollector creates an isolated collector for one source model.
func NewStreamCollector(format model.Format, exchange model.Exchange) (StreamCollector, error) {
	switch format {
...
func (c *anthropicCollector) Push(ctx context.Context, frame model.SSEFrame) error {
...
func (c *anthropicCollector) Close(ctx context.Context) (model.TransformResult, error) {
	if err := ctx.Err(); err != nil {
		return model.TransformResult{}, err
	}
	if !c.terminal {
		return model.TransformResult{}, protocolFailure(fmt.Errorf("Anthropic stream ended before message_stop"))
	}
...
func (c *chatCollector) Push(ctx context.Context, frame model.SSEFrame) error {
	if err := ctx.Err(); err != nil {
		return err
	}
	if c.terminal {
...
func (c *chatCollector) Close(ctx context.Context) (model.TransformResult, error) {
	if err := ctx.Err(); err != nil {
		return model.TransformResult{}, err
	}
	if !c.terminal {
		return model.TransformResult{}, protocolFailure(fmt.Errorf("Chat stream ended before [DONE]"))
	}
	...
	for _, index := range indices {
		...
		if !json.Valid([]byte(arguments)) {
			return model.TransformResult{}, protocolFailure(fmt.Errorf("Chat tool arguments are not valid JSON"))
		}
...
func (c *responsesCollector) Push(ctx context.Context, frame model.SSEFrame) error {
	if err := ctx.Err(); err != nil {
		return err
	}
	if c.terminal {
...
func (c *responsesCollector) Close(ctx context.Context) (model.TransformResult, error) {
	if err := ctx.Err(); err != nil {
		return model.TransformResult{}, err
	}
	if !c.terminal || c.response == nil {
		return model.TransformResult{}, protocolFailure(fmt.Errorf("Responses stream ended before response.completed"))
	}
	if model := c.exchange.OriginalRequest.Model; model != "" {
		c.response.Model = model
	}
