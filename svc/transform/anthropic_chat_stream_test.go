...
func task8Exchange(originalModel, translatedModel string) model.Exchange {
	next := 0
...
func task8FullChatFrames(callID string) []model.SSEFrame {
...
func task8PushAll(t *testing.T, stream StreamTransform, input []model.SSEFrame) []model.SSEFrame {
	t.Helper()
...
func task8Close(t *testing.T, stream StreamTransform, output *[]model.SSEFrame) error {
	t.Helper()
...
func task8EventNames(frames []model.SSEFrame) []string {
...
func task8DecodeData(t *testing.T, frame model.SSEFrame) map[string]any {
	t.Helper()
...
func task8FullAnthropicFrames(callID string) []model.SSEFrame {
	return []model.SSEFrame{
...
func TestChatToAnthropicStreamRejectsDoneWithoutFinishReason(t *testing.T) {
	...
	_, err = stream.Push(context.Background(), model.SSEFrame{Data: []byte(`[DONE]`)})
	var proxyErr *model.ProxyError
	require.ErrorAs(t, err, &proxyErr)
	assert.Equal(t, model.ERROR_PROTOCOL, proxyErr.Kind)
}
...
func TestAnthropicToChatStreamErrorTerminatesInChatShape(t *testing.T) {
	...
	output, err := stream.Push(context.Background(), model.SSEFrame{
		Event: "error",
...
func TestAnthropicChatStreamsRejectInvalidAccumulatedToolJSON(t *testing.T) {
	tests := []struct {
		name string
		new  func(model.Exchange) (StreamTransform, error)
		in   []model.SSEFrame
	}{
		{
