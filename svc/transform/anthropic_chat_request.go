...
func AnthropicToChatRequest(_ context.Context, env model.RequestEnvelope) (model.TransformResult, error) {
	...
	if len(src.StopSequences) > 0 {
		...
		if err != nil {
			return model.TransformResult{}, fmt.Errorf("encode chat stop sequences: %w", err)
		}
	}
	...
	return model.TransformResult{Body: body, Losses: thinkingLoss(src.Thinking)}, nil
}

// ChatToAnthropicRequest converts a Chat Completions request to Anthropic Messages.
func ChatToAnthropicRequest(_ context.Context, env model.RequestEnvelope) (model.TransformResult, error) {
	src, err := chat.DecodeRequest(env.Body)
	if err != nil {
		return model.TransformResult{}, invalidRequest("decode chat request", err)
	}
	...
	losses := make([]model.SemanticLoss, 0, 3)
	for _, message := range src.Messages {
		...
		case "system", "developer":
			...
			for _, block := range blocks {
				if block.Type != "text" {
					return model.TransformResult{}, unsupportedFeature("messages.content", "Anthropic system blocks support only text when translating Chat instructions")
				}
...
func chatMessageToAnthropic(message chat.Message) (anthropic.Message, []model.SemanticLoss, error) {
	var role string
	switch message.Role {
	case "user":
		role = "user"
...
func nameLoss(name string) []model.SemanticLoss {
	if name == "" {
		return nil
	}
...
func thinkingLoss(value *anthropic.Thinking) []model.SemanticLoss {
	if value == nil || value.BudgetTokens == 0 {
		return nil
	}
	return []model.SemanticLoss{{
