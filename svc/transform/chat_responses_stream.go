...
func NewChatToResponsesStream(exchange model.Exchange) (StreamTransform, error) {
...
func (s *chatToResponsesStream) Push(ctx context.Context, frame model.SSEFrame) ([]model.SSEFrame, error) {
...
func (s *chatToResponsesStream) Close(ctx context.Context) ([]model.SSEFrame, error) {
	if err := ctx.Err(); err != nil {
		return nil, err
	}
	if !s.terminal {
...
func (s *chatToResponsesStream) pushReasoning(delta string) ([]model.SSEFrame, error) {
...
func (s *chatToResponsesStream) pushText(delta string) ([]model.SSEFrame, error) {
	var frames []model.SSEFrame
	if s.messageIndex < 0 {
...
func (s *chatToResponsesStream) pushTool(delta chat.StreamToolCall) ([]model.SSEFrame, error) {
...
func (s *chatToResponsesStream) complete() ([]model.SSEFrame, error) {
...
func (s *chatToResponsesStream) event(name string, payload any) (model.SSEFrame, error) {
	body, err := json.Marshal(payload)
	if err != nil {
		return model.SSEFrame{}, protocolFailure(fmt.Errorf("encode %s: %w", name, err))
	}
	return model.SSEFrame{Event: name, Data: body}, nil
}
...
func NewResponsesToChatStream(exchange model.Exchange) (StreamTransform, error) {
	return &responsesToChatStream{exchange: exchange, toolIndices: make(map[string]int)}, nil
}

func (s *responsesToChatStream) Push(ctx context.Context, frame model.SSEFrame) ([]model.SSEFrame, error) {
	if err := ctx.Err(); err != nil {
		return nil, err
	}
	if s.terminal {
	...
	case "response.failed", "error":
		...
		s.terminal = true
		return []model.SSEFrame{{Data: errorBody}, {Data: []byte("[DONE]")}}, nil
...
func (s *responsesToChatStream) Close(ctx context.Context) ([]model.SSEFrame, error) {
	if err := ctx.Err(); err != nil {
		return nil, err
	}
	if !s.terminal {
...
func (s *responsesToChatStream) complete(frame model.SSEFrame) ([]model.SSEFrame, error) {
	var payload struct {
		Response responses.Response `json:"response"`
	}
	if err := json.Unmarshal(frame.Data, &payload); err != nil {
	...
	output = append(output, frames...)
	output = append(output, model.SSEFrame{Data: []byte("[DONE]")})
	s.terminal = true
	return output, nil
}

func (s *responsesToChatStream) chatFrames(delta chat.StreamDelta, finish string, usage *chat.Usage) ([]model.SSEFrame, error) {
	...
	return []model.SSEFrame{{Data: body}}, nil
}
