...
func TestTransformObserverRedactsDiagnosticDetails(t *testing.T) {
	...
	observer.RecordWarning(context.Background(), "xai", protocol.FORMAT_ANTHROPIC_MESSAGES, protocol.FORMAT_OPENAI_RESPONSES, protocol.Warning{
		Code: "downgrade", Message: "prompt secret Bearer token",
	})
	observer.RecordLoss(context.Background(), "xai", protocol.FORMAT_ANTHROPIC_MESSAGES, protocol.FORMAT_OPENAI_RESPONSES, protocol.SemanticLoss{
...
func TestTransformObserverIncrementsCounters(t *testing.T) {
	...
	observer.RecordWarning(context.Background(), "xai", protocol.FORMAT_OPENAI_CHAT, protocol.FORMAT_OPENAI_RESPONSES, protocol.Warning{Code: "warning"})
	observer.RecordLoss(context.Background(), "xai", protocol.FORMAT_OPENAI_CHAT, protocol.FORMAT_OPENAI_RESPONSES, protocol.SemanticLoss{Field: "messages.role"})
