...
func TestDefaultCatalogCapabilities(t *testing.T) {
	...
	tests := []struct {
		...
		{
			id: "minimax", preferred: protocol.FORMAT_ANTHROPIC_MESSAGES,
			baseURL:   "https://api.minimax.io/anthropic",
			endpoints: map[protocol.Format]string{protocol.FORMAT_ANTHROPIC_MESSAGES: "/v1/messages"},
		},
		{
			id: "openai-api", preferred: protocol.FORMAT_OPENAI_RESPONSES,
...
func TestXAINormalizerRejectsUnsupportedTools(t *testing.T) {
	...
	_, err = profile.NormalizeRequest(protocol.RequestEnvelope{
		TargetFormat: protocol.FORMAT_OPENAI_RESPONSES,
...
func TestOrdinaryNormalizerPreservesBody(t *testing.T) {
	...
	normalized, err := profile.NormalizeRequest(protocol.RequestEnvelope{Stream: true, Body: body})
	require.NoError(t, err)
...
func TestDefaultCatalogRoutingProfiles(t *testing.T) {
	...
	require.NoError(t, err)
	routed, err := router.Resolve(protocol.FORMAT_OPENAI_CHAT, "grok-4.5")
	require.NoError(t, err)
