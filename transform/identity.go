...
func AnthropicIdentity() Pair {
	return identityPair(protocol.FORMAT_ANTHROPIC_MESSAGES)
}

// ChatIdentity returns the validated Chat-to-Chat pair.
func ChatIdentity() Pair {
	return identityPair(protocol.FORMAT_OPENAI_CHAT)
}

// ResponsesIdentity returns the validated Responses-to-Responses pair.
func ResponsesIdentity() Pair {
	return identityPair(protocol.FORMAT_OPENAI_RESPONSES)
}

func identityPair(format protocol.Format) Pair {
	return newPair(
		format,
		format,
		identityRequest(format),
		identityResponse(format),
		func(protocol.Exchange) (StreamTransform, error) {
			return &identityStream{format: format}, nil
		},
	)
}

func identityRequest(format protocol.Format) RequestTransform {
	return func(ctx context.Context, envelope protocol.RequestEnvelope) (protocol.TransformResult, error) {
		...
		case protocol.FORMAT_ANTHROPIC_MESSAGES:
			...
			if err != nil {
				return protocol.TransformResult{}, invalidRequestError(err)
			}
		...
		default:
			return protocol.TransformResult{}, fmt.Errorf("identity request: unknown format %q", format)
		}
	}
}

func identityResponse(format protocol.Format) ResponseTransform {
	return func(ctx context.Context, envelope protocol.ResponseEnvelope) (protocol.TransformResult, error) {
		if err := ctx.Err(); err != nil {
			return protocol.TransformResult{}, err
		}
		switch format {
		...
		default:
			return protocol.TransformResult{}, fmt.Errorf("identity response: unknown format %q", format)
		}
	}
}

func encodeIdentity(body []byte, err error) (protocol.TransformResult, error) {
	if err != nil {
		return protocol.TransformResult{}, protocolFailure(err)
	}
...
func (s *identityStream) Push(ctx context.Context, frame protocol.SSEFrame) ([]protocol.SSEFrame, error) {
	if err := ctx.Err(); err != nil {
		return nil, err
	}
	if s.terminal {
...
func (s *identityStream) Close(ctx context.Context) ([]protocol.SSEFrame, error) {
	if err := ctx.Err(); err != nil {
		return nil, err
	}
	if !s.terminal {
...
func isTerminalFrame(format protocol.Format, frame protocol.SSEFrame) bool {
	switch format {
	case protocol.FORMAT_ANTHROPIC_MESSAGES:
