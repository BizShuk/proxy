...
func TestAnthropicToResponsesStreamMatchesNonStreamSemantics(t *testing.T) {
	...
	nonStream, err := AnthropicToResponsesResponse(context.Background(), protocol.ResponseEnvelope{
		Body: []byte(`{
...
func TestResponsesToAnthropicStreamFailedEvent(t *testing.T) {
	...
	output := task9PushAll(t, stream, []protocol.SSEFrame{{
...
func TestAnthropicResponsesStreamCloseRejectsMissingTerminal(t *testing.T) {
	constructors := []struct {
		...
		part protocol.SSEFrame
	}{
		{name: "anthropic to responses", new: NewAnthropicToResponsesStream, part: protocol.SSEFrame{Event: "message_start", Data: []byte(`{"type":"message_start","message":{"id":"msg_1","model":"claude-3"}}`)}},
		{name: "responses to anthropic", new: NewResponsesToAnthropicStream, part: protocol.SSEFrame{Event: "response.created", Data: []byte(`{"type":"response.created","response":{"id":"resp_1","model":"gpt-5"}}`)}},
	}
...
func task9ExchangeFor(originalModel, translatedModel string) protocol.Exchange {
	next := 0
...
func task9PushAll(t *testing.T, stream StreamTransform, input []protocol.SSEFrame) []protocol.SSEFrame {
	t.Helper()
...
func task9CloseStream(t *testing.T, stream StreamTransform, output *[]protocol.SSEFrame) error {
	t.Helper()
...
func task9EventNames(frames []protocol.SSEFrame) []string {
...
func task9DecodeEvent(t *testing.T, frame protocol.SSEFrame, target any) {
	t.Helper()
	require.NoError(t, json.Unmarshal(frame.Data, target))
}

func task9CompletedResponse(t *testing.T, frames []protocol.SSEFrame) responses.Response {
	t.Helper()
	for _, frame := range frames {
...
func task9FullResponsesFrames() []protocol.SSEFrame {
...
func task9FullAnthropicFrames() []protocol.SSEFrame {
	return []protocol.SSEFrame{
