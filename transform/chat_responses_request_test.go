...
func TestChatResponsesPreserveParallelToolCalls(t *testing.T) {
	chatResult, err := ChatToResponsesRequest(context.Background(), protocol.RequestEnvelope{
		Model: "target",
		Body: []byte(`{
			"model":"source",
	...
	responsesResult, err := ResponsesToChatRequest(context.Background(), protocol.RequestEnvelope{
		Model: "target",
		Body: []byte(`{
			"model":"source",
			"input":"hi",
...
func TestChatToResponsesReportsDeveloperPriorityLoss(t *testing.T) {
	result, err := ChatToResponsesRequest(context.Background(), protocol.RequestEnvelope{
		Model: "target-model",
		Body: []byte(`{
			"model":"source-chat",
			"messages":[
...
func TestResponsesToChatRejectsNonportableState(t *testing.T) {
	_, err := ResponsesToChatRequest(context.Background(), protocol.RequestEnvelope{
		Model: "target-model",
...
func TestChatResponsesRequestTransformsHonorCancellation(t *testing.T) {
	...
	for _, fn := range []RequestTransform{ChatToResponsesRequest, ResponsesToChatRequest} {
		_, err := fn(ctx, protocol.RequestEnvelope{Body: []byte(`{}`)})
		require.ErrorIs(t, err, context.Canceled)
	}
}

func TestChatToResponsesRejectsUnsupportedMessageRole(t *testing.T) {
	_, err := ChatToResponsesRequest(context.Background(), protocol.RequestEnvelope{
		Model: "target-model",
...
func TestChatResponsesInvalidJSONUsesTypedError(t *testing.T) {
	for _, fn := range []RequestTransform{ChatToResponsesRequest, ResponsesToChatRequest} {
		_, err := fn(context.Background(), protocol.RequestEnvelope{Model: "target", Body: []byte(`{`)})
		var proxyErr *protocol.ProxyError
		require.ErrorAs(t, err, &proxyErr)
