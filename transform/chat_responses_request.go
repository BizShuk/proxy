...
func ChatToResponsesRequest(ctx context.Context, envelope protocol.RequestEnvelope) (protocol.TransformResult, error) {
	if err := ctx.Err(); err != nil {
		return protocol.TransformResult{}, err
	}
	source, err := chat.DecodeRequest(envelope.Body)
	if err != nil {
		return protocol.TransformResult{}, chatResponsesInvalidRequest("decode Chat request", err)
	}
	...
	for index, message := range source.Messages {
		...
		case "system", "developer":
			...
			if err != nil {
				return protocol.TransformResult{}, fmt.Errorf("convert Chat messages[%d]: %w", index, err)
			}
		...
		case "tool":
			if message.ToolCallID == "" {
				return protocol.TransformResult{}, chatResponsesInvalidRequest(
		...
		default:
			return protocol.TransformResult{}, chatResponsesUnsupported(
...
func ResponsesToChatRequest(ctx context.Context, envelope protocol.RequestEnvelope) (protocol.TransformResult, error) {
	if err := ctx.Err(); err != nil {
		return protocol.TransformResult{}, err
	}
	source, err := responses.DecodeRequest(envelope.Body)
	if err != nil {
		return protocol.TransformResult{}, chatResponsesInvalidRequest("decode Responses request", err)
	}
	...
	if err != nil {
		return protocol.TransformResult{}, chatResponsesInvalidRequest("decode Responses input", err)
	}
	...
	for index, item := range input {
		...
		case "message":
			...
			if err != nil {
				return protocol.TransformResult{}, fmt.Errorf("convert Responses input[%d]: %w", index, err)
			}
...
func chatResponsesInstructionText(message chat.Message) (string, []protocol.SemanticLoss, error) {
...
func chatResponsesMessageItems(message chat.Message) ([]responses.InputItem, []protocol.SemanticLoss, error) {
	if message.ToolCallID != "" {
...
func chatResponsesRequestLosses(source *chat.Request) []protocol.SemanticLoss {
	losses := make([]protocol.SemanticLoss, 0, 5)
...
func chatResponsesNameLoss(index int) protocol.SemanticLoss {
...
func chatResponsesContent(role string, content *chat.MessageContent) (responses.ContentList, []protocol.SemanticLoss, error) {
	if content == nil {
		return nil, nil, nil
	}
	textType := "input_text"
