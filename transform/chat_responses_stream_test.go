...
func TestChatToResponsesStreamIsIncrementalAndCollectable(t *testing.T) {
	...
	require.NoError(t, err)
	assertResponseSemantics(t, protocol.FORMAT_OPENAI_RESPONSES, result.Body, responseSemantics{
...
func TestResponsesToChatStreamIsIncrementalAndCollectable(t *testing.T) {
	...
	result, err := collector.Close(context.Background())
	require.NoError(t, err)
	assertResponseSemantics(t, protocol.FORMAT_OPENAI_CHAT, result.Body, responseSemantics{
		text: "done", reasoning: "checked", toolName: "read", callID: "call_1",
		inputTokens: 10, outputTokens: 4, stop: "tool",
	})
}
...
func streamTestExchange(originalModel, translatedModel string) protocol.Exchange {
	next := 0
...
func streamEventNames(frames []protocol.SSEFrame) []string {
...
func chatResponsesFullResponsesFrames() []protocol.SSEFrame {
